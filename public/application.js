...
/***/ (function(module, exports, __webpack_require__) {
	...
	var validate = function validate(input) {
	  if (/[a-z]/.test(input.toLowerCase()) === false) {
	    return false;
	  }
	  return true;
	};
	...
	var onDeleteQuestion = function onDeleteQuestion(event) {
	  ...
	  api.deleteQuestion(questionId).then(ui.deleteQuestionSuccess).catch(ui.deleteQuestionFailure);
	};

	var onEditQuestion = function onEditQuestion(event) {
	  event.preventDefault();
	  ...
	  api.editQuestion(questionId, data).then(ui.editQuestionSuccess).catch(ui.editQuestionFailure);
	  // } else {
	  //   ui.editQuestionFailure()
	  // }
	};

	var onAddQuestion = function onAddQuestion(event) {
	  ...
	  api.addQuestion(surveyId, data).then(ui.addQuestionSucess).catch(ui.addQuestionFailure);
	};

	var onShowUserSurveyTaken = function onShowUserSurveyTaken(event) {
	  event.preventDefault();
	  var userId = $(this).attr('userId');
	  ...
	  api.showUserSurveyTaken(surveyId).then(ui.showUserSurveyTakenSuccess).catch(ui.showUserSurveyTakenFailure);
	};
	...
	var addHandlers = function addHandlers() {
	  ...
	  $('#content').on('click', '.delete-question-button', onDeleteQuestion), $('#content').on('submit', '.edit-question', onEditQuestion);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var editQuestion = function editQuestion(questionId, data) {
	...
	var addQuestion = function addQuestion(surveyId, data) {
	...
	var showUserSurveyTaken = function showUserSurveyTaken(surveyId, data) {
	...
	module.exports = {
	  ...
	  showUserSurveyTaken: showUserSurveyTaken,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var resetSurveyFormFields = function resetSurveyFormFields() {
	  $('form#sign-up').trigger('reset');
	  $('form#sign-in').trigger('reset');
	  $('form#sign-out').trigger('reset');
	  $('form#change-password').trigger('reset');
	...
	var deleteQuestionSuccess = function deleteQuestionSuccess() {
	  api.showAuthUserSurveys().then(showAuthUserSurveysSuccess).catch(showAuthUserSurveysFailure);
	  $('.alert').text('Question Deleted');
	};

	var deleteQuestionFailure = function deleteQuestionFailure(questionId) {
	  $('.alert').text('Failed to Delete Question');
	};

	var editQuestionSuccess = function editQuestionSuccess(data) {
	  // console.log('edit question success fired for question', question.id)
	  api.surveyQuestions().then(surveyQuestionsSuccess).catch(surveyQuestionsFailure);
	  resetSurveyFormFields();
	};

	var editQuestionFailure = function editQuestionFailure() {
	  // console.log('edit question failure data is', data)
	  resetSurveyFormFields();
	  $('.alert').text('Failed to Update Question');
	};

	var addQuestionSuccess = function addQuestionSuccess(response) {
	  console.log('add question success this is response.question', response.question);
	  // store.surveyID = response.question.prompt
	  var showUserQuestionHtml = authUserSurveyHB({ questions: response.question.prompt });
	  $('#content').append(showUserQuestionHtml);
	  document.getElementById('add-question').reset();
	  resetSurveyFormFields();
	};

	var addQuestionFailure = function addQuestionFailure() {
	  console.log('add question failure');
	  resetSurveyFormFields();
	  $('.alert').text('Failed to Add a Question');
	};

	var showUserSurveyTakenSuccess = function showUserSurveyTakenSuccess(data) {
	  $('#content').hide();
	  $('form').hide();
	  $('.alert').text('');
	  var answerableSurveyHtml = authUserSurveyHB({ surveys: data.survey });
	...
	var showUserSurveyTakenFailure = function showUserSurveyTakenFailure(data) {
	  $('.alert').text('Unable to Retrieve Data.');
	  setTimeout(function () {
	    $('.alert').text('');
	  }, 4000);
	...
	module.exports = {
	  ...
	  editQuestionFailure: editQuestionFailure,
	  showUserSurveyTakenSuccess: showUserSurveyTakenSuccess,
	  showUserSurveyTakenFailure: showUserSurveyTakenFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td class=\"prompt-title create\">"
	    ...
	    + "\" type=\"click\" class=\"btn btn-danger delete-question-button\">Delete</button></td>\n\n        <td>\n          <form class=\"edit-question\" questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n          <fieldset>\n            <input type=\"text\" name=\"question[prompt]\" prompt=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.prompt : stack1), depth0))
	    + "\" placeholder=\"Edit Prompt\">\n        <input type=\"submit\" class=\"btn btn-info\" name=\"submit\" value=\"Update Question\">\n      </fieldset>\n      </form>\n      </tr>\n\n";
